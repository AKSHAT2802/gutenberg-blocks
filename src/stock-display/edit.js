                    ...
                    <tbody>
                        {stocks.map(stock => (
                            <tr key={stock.id}>
                                ...
                                <td className={`stock-chg ${stock.chg >= 0 ? 'positive' : 'negative'}`}>
                                    {stock.chg >= 0 ? '+' : ''}{stock.chg}
                                </td>
                                <td className={`stock-percent ${stock.chg_percent >= 0 ? 'positive' : 'negative'}`}>
                                    {stock.chg_percent >= 0 ? '+' : ''}{stock.chg_percent}%
                                </td>
                                <td className="stock-ratio">{stock.ad_ratio}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
