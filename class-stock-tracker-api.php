<?php
/**
 * Class to handle API calls for stock tracker block.
 *
...
class Stock_Tracker_API {
	...
	/**
	 * Insert_sample_data function to add data to table.
	 *
	 * @return void
	 */
	...
	public function register_endpoints() {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'get_stocks' ),
		...
		register_rest_route(
			'stock-tracker/v1',
			'/stocks/(?P<category>[a-zA-Z]+)',
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'get_stocks_by_category' ),
				'permission_callback' => '__return_true',
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'bulk_update_stocks' ),
				'permission_callback' => function () {
					return current_user_can( 'manage_options' );
				},
			)
	...
	public function update_stock( $request ) {
		...
		$required_fields = array( 'id', 'ltp', 'chg', 'chg_percent', 'ad_ratio' );
